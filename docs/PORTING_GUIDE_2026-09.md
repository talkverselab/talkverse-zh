...
### 2-1d. 말하기 연습 (Speaking) — 학습한 회화 → 탭 즉시 테스트
- 메뉴: 홈 `말하기` → `SpeakingPracticeScreen` 목록. 에피소드 화면에서 학습 체크(userProgress learned/reviewCount)한 턴이 있는 회화만 표시, 하나도 없으면 전체 회화. 상단에서 단계(1단계 10초 / 2단계 5초 / 3단계 2초) 선택(`speak_stage` 저장).
- 회화를 탭하면 `SpeakingTestScreen`이 TEST 버튼 없이 바로 시작: 한국어 문장 표시 → 1.2초 뒤 자동 녹음 → 제한 시간 안에 문장 전체를 말하면 PASS → 0.9초 뒤 다음 문장. 4문장마다 결과 화면(✓/✗·인식 문장·듣기) + `계속` 버튼, 마지막엔 전체 요약 + `처음부터 다시`.
- 힌트(목표어 문장 + 발음) 토글 `speak_show_hint`, 발음은 `KoReadingText`로 전역 독음 토글 연동.
- 오픈소스: `speech_to_text` ^7.4 (BSD-3, Android SpeechRecognizer 스트리밍 partial) + `lpinyin` ^2.0 (MIT). 오프라인 대안 sherpa-onnx (Apache-2.0).
- 판정 `speak_match.dart`: 인식 한자→무성조 병음 → 퍼지(zh/ch/sh→z/c/s, n·r→l, ing/eng→in/en) → 목표 대비 LCS ≥ 0.7 PASS(≤3음절은 전부). 부분 결과마다 즉시 판정, 시간 종료 시 `stop()` 후 최종 결과 최대 1.2초 대기.
- 저장: `speak_best_<turnId>` = 통과 단계(1~3). 목록에 '3단계 통과 n' 표시.
- Android: `RECORD_AUDIO`·`INTERNET`·`BLUETOOTH_CONNECT` + `<queries>` `android.speech.RecognitionService`.
